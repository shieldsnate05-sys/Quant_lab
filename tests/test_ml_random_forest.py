...
def test_predict_and_predict_proba_shapes(
    ...
    X, y = toy_data
    model = RandomForestModel(RandomForestConfig(n_estimators=20)).fit(X, y)
...
def test_handles_negative_and_zero_labels_directly() -> None:
...
def test_feature_importances_sum_and_order(
    toy_data: tuple[pd.DataFrame, pd.Series],
) -> None:
    X, y = toy_data
    model = RandomForestModel(RandomForestConfig(n_estimators=50)).fit(X, y)
